...
prop_idReturnsArg (WellTyped term) = isRight $ testingCtx >>= flip runTC (do
  ty <- typeCheck term
...
prop_pairFstReturnsArg (WellTyped term) = isRight $ testingCtx >>= flip runTC (do
  ty <- typeCheck term
...
prop_pairSndReturnsArg (WellTyped term) = isRight $ testingCtx >>= flip runTC (do
  ty <- typeCheck term
...
prop_etaExpansion (WellTyped term) (WellTyped arg) v = isRight $ testingCtx >>= flip runTC (do
  ty <- typeCheck arg
